...
def extract_rebirth_installer(installer_path, dest_dir):
    ...
    if seven_zip:
        ...
        if result.returncode == 0:
            ...
            return False, None, "7-Zip finished but Rebirth.exe was not found in extracted files."
    ...
    rebirth_exe = find_rebirth_exe_in_dir(dest_dir)
    if rebirth_exe:
        return True, rebirth_exe, "extracted_with_inno"
    if inno_result.returncode == 0:
        return False, None, "Installer finished but Rebirth.exe was not found."
    detail = (inno_result.stderr or inno_result.stdout or "").strip()
    if seven_zip:
        return False, None, detail or "Extraction failed."
    return False, None, (
        "Could not extract automatically. Install 7-Zip, or run the installer manually.\n"
        + (detail or "")
    )

LAUNCHER_MAIN_FILE = "!start.py"
...
class ModernRebirthStudioToolBox:
    ...
    def refresh_start_launch_info(self):
        ...
        if is_rebirth_exe_ready():
            ...
            self.lbl_start_iso_hint.config(
                text="Open Get ReBirth → Step 2: run installer, then deploy Rebirth.exe into the ToolBox folder.",
    ...
    def build_ui(self):
        ...
        tk.Label(self.step2_card, text="Install ReBirth", font=("Segoe UI", 10, "bold"), fg="#FFFFFF", bg="#12131A", anchor="w").pack(fill=tk.X, padx=10, pady=(0, 8))
        self.lbl_step2_status = tk.Label(
            self.step2_card,
            text=f"ReBirth will be installed directly into:\n{BASE_DIR}",
            font=("Segoe UI", 8),
            fg="#A0A5C0",
            bg="#12131A",
            anchor="w",
    ...
    def refresh_get_rebirth_ui(self):
        ...
        if step2_done:
            ...
            self.lbl_step2_status.config(
                text=f"Install ReBirth directly into:\n{install_dir}\n\nWaiting for Rebirth.exe…",
                fg="#A0A5C0",
            )
        ...
        if step2_done and self.setup_wizard_only:
            self.set_wizard_message("ReBirth installed — restart ToolBox to unlock all tabs.", "#00FF66")
        elif step2_done:
            self.set_wizard_message("Setup complete — ReBirth is ready in this folder.", "#00FF66")
        elif step1_done:
            self.set_wizard_message("Step 1 complete — click Install ReBirth in Step 2.", "#00E5FF")
        else:
            self.set_wizard_message("Download the ISO and installer, then install ReBirth here.", "#A0A5C0")
    ...
    def run_rebirth_installer(self):
        ...
        save_config(self.config)
        try:
            subprocess.Popen([status["installer_path"], f"/DIR={install_dir}"])
            self.set_wizard_message(
                f"Installer opened — install ReBirth into this ToolBox folder:\n{install_dir}",
                "#00E5FF",
            )
            self.lbl_step2_status.config(
                text=f"Install to:\n{install_dir}\n\nWaiting for Rebirth.exe…",
    ...
    def extract_rebirth_portable(self):
        ...
        install_dir = self.ensure_rebirth_install_folder()
        if not install_dir:
            return

        self.show_loading_overlay("Extracting ReBirth…", f"Unpacking into:\n{install_dir}")
        ...
        def worker():
            try:
                ...
                def done():
                    self.hide_loading_overlay()
                    self.set_wizard_message(
                        f"Step 2 complete — ReBirth extracted to {install_dir}",
                        "#00FF66",
                    )
                    self.refresh_get_rebirth_ui()
